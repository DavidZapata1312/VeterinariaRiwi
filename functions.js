...
function buscarMascota() {
  ...
  let resultado = pacientes.find(
    (paciente) => paciente.nombre === nombrebuscado
  );
  if (resultado) {
    console.log(`nombre: ${resultado.nombre}`);
    console.log(`especie: ${resultado.especie}`);
    console.log(`edad: ${resultado.edad}`);
    console.log(`Vacunado: ${resultado.vacunado ? "Sí" : "No"}`);
  }
}
buscarMascota();
...
function calcularPromedioEdad(pacientes) {
  ...
  let sumaEdades = pacientes.reduce(
    (total, paciente) => total + paciente.edad,
    0
  );
